...
function SidebarFilterOne(props) {
    ...
    useEffect(() => {
        ...
        if (isFirst) {
            ...
            console.log('got bug');
            console.log('got bug Query', query);
            //scrollTopHandler();
        }
    }, [])
                        ...
                        <div className="sticky-sidebar">
                            {
                                type === "boxed" || type === "banner" ? '' :
                                    <div className="filter-actions mb-4">
                                        ...
                                        <ALink href={{ pathname: router.pathname, query: { grid: query.get('grid') } }} scroll={false} className="filter-clean">Clean All</ALink>
                                    </div>
                            }
                                    ...
                                    <ul className="widget-body filter-items search-ul">
                                        {
                                            data && sidebarData.categories.map((item, index) => (
                                                item.children ?
                                                    <li
                                                        key={item.name + ' - ' + index}
                                                        className={`with-ul overflow-hidden ${item.slug === query.get('category') || item.children.findIndex(subCat => subCat.slug === query.get('category')) > -1 ? 'show' : ''} `}
                                                    >
                                                        <SlideToggle collapsed={true} >
                                                            {({ onToggle, setCollapsibleElement, toggleState }) => (
                                                                <>
                                                                    <ALink href={{ pathname: router.pathname, query: { category: item.slug, grid: query.get('grid') } }} scroll={false}>{item.name}
                                                                            ...
                                                                            <ul style={{ display: "block" }}>
                                                                                {
                                                                                    item.children.map((subItem, index) =>
                                                                                        <li key={subItem.name + ' - ' + index}
                                                                                            className={`with-ul ${subItem.slug === query.get('category') ? 'show' : ''} `}>
                                                                                            <ALink scroll={false} href={{ pathname: router.pathname, query: { category: subItem.slug, grid: query.get('grid') } }}>{subItem.name}</ALink>
                                                                                        </li>
                                                                                    )}
                                                                            </ul>
                                                                        </div>
                                                    ...
                                                    <li
                                                        className={query.get('category') === item.slug ? 'show' : ''}
                                                        key={item.name + ' - ' + index}
                                                    >
                                                        <ALink href={{ pathname: router.pathname, query: { category: item.slug, grid: query.get('grid') } }} scroll={false}>{item.name}
                                                        </ALink>
